        ...
        <div class="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12 space-y-8">
            <section class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="rounded-[24px] bg-gradient-to-br from-slate-950 via-[#4d255f] to-[#8f52a3] p-6 text-white editorial-shadow">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#eadff1]">Top Course</p>
                    <p class="mt-3 font-headline text-2xl font-bold">{{ $topCourseTitle }}</p>
                    <p class="mt-2 text-sm text-[#eadff1]/85">{{ $topCourseStudents }} enrolled students</p>
                </div>
            </section>
            ...
            <section class="rounded-[28px] bg-white border border-slate-200 overflow-hidden editorial-shadow">
                <div class="px-6 md:px-8 py-6 border-b border-slate-200 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div class="flex gap-3 flex-wrap">
                        <span class="rounded-full bg-[#f5eef8] px-4 py-2 text-sm font-semibold text-[#6a3378]">All Students {{ $students->total() }}</span>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($students as $studentRow)
                                @php
                                    ...
                                    $progressClass = $progressPercent >= 85 ? 'bg-emerald-500' : ($progressPercent <= 35 ? 'bg-amber-500' : 'bg-[#f5eef8]0');
                                    $statusClass = $progressPercent >= 85 ? 'bg-emerald-50 text-emerald-700' : ($progressPercent <= 35 ? 'bg-amber-50 text-amber-700' : 'bg-[#f5eef8] text-[#6a3378]');
                                @endphp
                                <tr class="hover:bg-slate-50/60 transition-colors">
                                    <td class="px-6 md:px-8 py-5">
                                        <div class="flex items-center gap-3">
                                            <img alt="Student Avatar" class="w-11 h-11 rounded-2xl object-cover" src="{{ $enrollment->user?->avatarUrl(96) }}" />
                                            <div>
