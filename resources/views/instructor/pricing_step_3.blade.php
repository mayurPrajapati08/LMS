...
<head>
    ...
    <title>Pricing Setup - Scholarly Editorial</title>
        ...
        <div class="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-12 space-y-10">
            <section class="rounded-[28px] bg-gradient-to-br from-slate-900 via-indigo-900 to-violet-800 text-white overflow-hidden editorial-shadow">
                <div class="grid gap-8 lg:grid-cols-[1.35fr_0.95fr] p-6 md:p-10">
                    <div class="space-y-6">
                        <div class="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-indigo-100">Step 3 of 4</div>
                        <div class="space-y-3">
                            ...
                            <p class="max-w-2xl text-sm md:text-base text-indigo-100/90 leading-relaxed">Choose the access model, duration, and learner-facing pricing details. This is where your course becomes a product, not just content.</p>
                        </div>
                    </div>
                    <div class="rounded-[24px] border border-white/15 bg-white/10 p-5 md:p-6 backdrop-blur-sm">
                        ...
                        <div class="mt-5 space-y-4">
                            <div class="flex items-center gap-4 opacity-85"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary font-bold shadow-lg">1</div><div><p class="text-sm font-bold text-white">Basic Info</p><p class="text-xs text-indigo-100/80">Completed</p></div></div>
                            <div class="flex items-center gap-4 opacity-85"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary font-bold shadow-lg">2</div><div><p class="text-sm font-bold text-white">Curriculum</p><p class="text-xs text-indigo-100/80">Completed</p></div></div>
                            <div class="flex items-center gap-4"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary font-bold shadow-lg">3</div><div><p class="text-sm font-bold text-white">Pricing</p><p class="text-xs text-indigo-100/80">Current step</p></div></div>
                            <div class="flex items-center gap-4 opacity-60"><div class="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/20 bg-white/5 font-bold text-indigo-100">4</div><div><p class="text-sm font-semibold text-indigo-50">Review</p><p class="text-xs text-indigo-100/70">Final check</p></div></div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="rounded-[28px] bg-white p-6 md:p-8 editorial-shadow">
                <div class="flex flex-wrap items-center gap-4 md:gap-6">
                    <div class="flex items-center gap-3 opacity-70"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary font-bold shadow-lg border border-indigo-100">1</div><div><p class="text-sm font-bold text-slate-900">Basic Info</p><p class="text-xs text-slate-500">Done</p></div></div>
                    <div class="hidden md:block h-px flex-1 bg-slate-200"></div>
                    <div class="flex items-center gap-3 opacity-70"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary font-bold shadow-lg border border-indigo-100">2</div><div><p class="text-sm font-bold text-slate-900">Curriculum</p><p class="text-xs text-slate-500">Done</p></div></div>
                ...
                <div class="col-span-12 xl:col-span-8 space-y-8">
                    <div class="rounded-[28px] bg-white p-6 md:p-8 editorial-shadow space-y-8">
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <label class="rounded-[24px] {{ $selectedPricingModel === 'one_time' ? 'border-2 border-primary bg-indigo-50' : 'border border-slate-200 bg-white hover:border-slate-300' }} p-5 cursor-pointer transition-colors">
                            ...
                            <label class="rounded-[24px] {{ $selectedPricingModel === 'subscription' ? 'border-2 border-primary bg-indigo-50' : 'border border-slate-200 bg-white hover:border-slate-300' }} p-5 cursor-pointer transition-colors">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input class="w-full rounded-2xl bg-slate-50 border {{ $errors->has('price') ? 'border-red-300 focus:border-red-300 focus:ring-red-100' : 'border-slate-200 focus:border-indigo-300 focus:ring-indigo-100' }} pl-16 pr-5 py-4 text-on-surface outline-none transition-all" name="price" placeholder="2999" type="number" min="0" step="0.01" value="{{ old('price', $pricingCourse?->price) }}" />
                                </div>
                                @error('price')
                            ...
                            <div class="space-y-2 {{ $selectedPricingModel === 'subscription' ? 'hidden' : '' }}" id="validityField">
                                ...
                                <select class="w-full rounded-2xl bg-slate-50 border border-slate-200 px-5 py-4 text-on-surface outline-none transition-all focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100 appearance-none" name="validity_in_days">
                            ...
                            <div class="space-y-2 {{ $selectedPricingModel === 'subscription' ? '' : 'hidden' }}" id="subscriptionCycleField">
                                ...
                                <select class="w-full rounded-2xl bg-slate-50 border {{ $errors->has('subscription_cycle') ? 'border-red-300 focus:border-red-300 focus:ring-red-100' : 'border-slate-200 focus:border-indigo-300 focus:ring-indigo-100' }} px-5 py-4 text-on-surface outline-none transition-all focus:ring-4 appearance-none" name="subscription_cycle">
                            ...
                            <div class="space-y-2 md:col-span-2">
                                ...
                                <input class="w-full rounded-2xl bg-slate-50 border border-slate-200 px-5 py-4 text-on-surface outline-none transition-all focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100 placeholder:text-slate-400" name="promotional_note" placeholder="Optional line shown during launch, e.g. Early-bird pricing for the first 100 learners" type="text" value="{{ $promotionalNote }}" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-[28px] bg-white p-6 md:p-8 editorial-shadow space-y-5">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="rounded-[24px] bg-gradient-to-br from-primary to-primary-container p-6 text-white">
                                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-100">Suggested Public Price</p>
                            <p class="mt-3 font-headline text-4xl font-bold">Rs. {{ number_format((float) old('price', $pricingCourse?->price ?? 0), 0) }}</p>
                            <p class="mt-2 text-sm text-indigo-100/85" id="pricingSnapshotMessage">
